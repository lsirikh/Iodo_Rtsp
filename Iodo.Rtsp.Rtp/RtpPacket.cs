...
internal struct RtpPacket
{
	...
	public static bool TryParse(ArraySegment<byte> byteSegment, out RtpPacket rtpPacket)
	{
		...
		if (rtpPacket.ExtensionFlag)
		{
			...
			offset += 2;
			int num = BigEndianConverter.ReadUInt16(byteSegment.Array, offset) * 4;
			offset += 2 + num;
		}
		int num2 = byteSegment.Offset + byteSegment.Count - offset;
		if (rtpPacket.PaddingFlag)
		{
			int num3 = byteSegment.Array[byteSegment.Offset + byteSegment.Count - 1];
			num2 -= num3;
		}
		rtpPacket.PayloadSegment = new ArraySegment<byte>(byteSegment.Array, offset, num2);
		return true;
	}
}

...
internal class RtspHttpTransportClient : RtspTransportClient
{
	...
	public override async Task ConnectAsync(CancellationToken token)
	{
		...
		await SocketTaskExtensions.ConnectAsync(port: (connectionUri.Port != -1) ? connectionUri.Port : 80, socket: _streamDataClient, host: connectionUri.Host);
		_remoteEndPoint = _streamDataClient.RemoteEndPoint;
		_dataNetworkStream = new NetworkStream(_streamDataClient, ownsSocket: false);
		...
		MemoryStream ms = new MemoryStream(buffer, 0, await ReadUntilEndOfHeadersAsync(_dataNetworkStream, buffer));
		StreamReader streamReader = new StreamReader(ms, Encoding.ASCII);
		string responseLine = streamReader.ReadLine();
		if (responseLine == null)
		{
			throw new HttpBadResponseException("Empty response");
		}
		string[] tokens = responseLine.Split(new char[1] { ' ' });
		if (tokens.Length != 3)
		{
		...
		{
			if (statusCode != HttpStatusCode.Unauthorized || ConnectionParameters.Credentials.IsEmpty() || _authenticator != null)
			{
	...
	protected override async Task WriteAsync(byte[] buffer, int offset, int count)
	{
		...
		{
			Uri connectionUri = ConnectionParameters.ConnectionUri;
			await SocketTaskExtensions.ConnectAsync(port: (connectionUri.Port != -1) ? connectionUri.Port : 80, socket: _commandsClient, host: connectionUri.Host);
			...
			byte[] requestBytes = Encoding.ASCII.GetBytes(request);
			await SocketTaskExtensions.SendAsync(buffers: new ArraySegment<byte>[2]
			{
				new ArraySegment<byte>(requestBytes),
				new ArraySegment<byte>(base64CommandBytes)
			}, socket: _commandsClient, socketFlags: SocketFlags.None);
		}
	}
	...
	private string GetAuthorizationHeader(uint counter, string method, byte[] requestBytes)
	{
		...
		{
			string response = _authenticator.GetResponse(counter, ConnectionParameters.ConnectionUri.PathAndQuery, method, requestBytes);
	...
	private static async Task<int> ReadUntilEndOfHeadersAsync(Stream stream, byte[] buffer)
	{
		...
		do
		{
			...
			endOfHeaders = ArrayUtils.IndexOfBytes(buffer, Constants.DoubleCrlfBytes, startIndex, totalRead - startIndex);
			offset += read;
		}

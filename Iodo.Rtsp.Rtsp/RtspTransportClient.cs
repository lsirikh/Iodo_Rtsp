...
using Iodo.Rtsp.Rtsp.Authentication;
using Iodo.Rtsp.Utils;

namespace Iodo.Rtsp.Rtsp;

internal abstract class RtspTransportClient : IRtspTransportClient, IDisposable
{
	...
	public async Task<RtspResponseMessage> ExecuteRequest(RtspRequestMessage requestMessage, CancellationToken token, int responseReadPortionSize = 0)
	{
		...
		RtspResponseMessage responseMessage2 = await GetResponseAsync(responseReadPortionSize);
		if (responseMessage2.StatusCode != RtspStatusCode.Unauthorized)
		{
			return responseMessage2;
		}
		if (ConnectionParameters.Credentials.IsEmpty() || _authenticator != null)
		{
			throw new RtspBadResponseCodeException(responseMessage2.StatusCode);
		}
		string authenticateHeader = responseMessage2.Headers[WellKnownHeaders.WwwAuthenticate];
		...
		await SendRequestAsync(requestMessage, token);
		responseMessage2 = await GetResponseAsync();
		if (responseMessage2.StatusCode == RtspStatusCode.Unauthorized)
		{
			throw new RtspBadResponseCodeException(responseMessage2.StatusCode);
		}
	...
	public Task SendRequestAsync(RtspRequestMessage requestMessage, CancellationToken token)
	{
		...
		string text = requestMessage.ToString();
		int bytes = Encoding.ASCII.GetBytes(text, 0, text.Length, _buffer, 0);
		return WriteAsync(_buffer, 0, bytes);
	}
	...
	private async Task<RtspResponseMessage> GetResponseAsync(int responseReadPortionSize = 0)
	{
		...
		int endOfResponseHeaders = ArrayUtils.IndexOfBytes(_buffer, Constants.DoubleCrlfBytes, 0, totalRead) + Constants.DoubleCrlfBytes.Length;
		if (endOfResponseHeaders == -1)
		{
			throw new RtspBadResponseException("End of response headers is not found");
		}
		ArraySegment<byte> headersByteSegment = new ArraySegment<byte>(_buffer, startOfResponse, endOfResponseHeaders - startOfResponse);
		...
		if (!uint.TryParse(contentLengthString, out var contentLength))
		{
	...
	private async Task<int> ReadUntilEndOfHeadersAsync(int readPortionSize = 0)
	{
		...
		do
		{
			...
			if (count == 0)
			{
				throw new RtspBadResponseException($"Response is too large (> {8} KB)");
			}
			...
			endOfHeaders = ArrayUtils.IndexOfBytes(_buffer, Constants.DoubleCrlfBytes, startIndex, totalRead - startIndex);
			offset += read;
		}
	...
	private void AddAuthorizationHeader(RtspRequestMessage request)
	{
		...
		string response = _authenticator.GetResponse(request.CSeq, fixedRtspUri.ToString(), request.Method.ToString(), Array.Empty<byte>());
